...
// --- ActivityItem (fixed duplicates and added empreendimento select) ---
const ActivityItem = ({ plano, dayKey, onDelete, onUpdate, executorMap, allPlanejamentos, provided, isDragging, isReprogramando, isSelected, onToggleSelect, hasSelections }) => {
  ...
  const displayName = useMemo(() => {
    if (plano.tipo_planejamento === 'documento') {
      const numeroFolha = plano.documento?.numero || (plano.descritivo && plano.descritivo.includes(' - ') ? plano.descritivo.split(' - ')[0] : null) || 'Número';
      const nomeArquivo = plano.documento?.arquivo || (plano.descritivo && plano.descritivo.includes(' - ') ? plano.descritivo.split(' - ')[1] : null) || 'Documento';
  ...
  const handleStartActivity = async () => {
    if (activeExecution) { alert("Uma atividade já está em progresso. Pare a atividade atual antes de iniciar uma nova."); return; }
    if (realStatus === 'concluido') { alert("Esta atividade já foi concluída e não pode ser iniciada novamente."); return; }
    if (plano.isLegacyExecution) { alert("Esta é uma atividade executada sem planejamento (antiga) e não pode ser iniciada novamente."); return; }
  ...
  const handleAdjustTime = async () => {
    ...
    if (isNaN(timeValue) || timeValue < 0) { alert("Por favor, insira um tempo válido."); return; }
    try {
      if (plano.isLegacyExecution) { alert("Não é possível ajustar o tempo para atividades rápidas antigas (não planejadas)."); return; }
  ...
  const shouldShowDeleteButton = () => hasPermission('admin') || hasPermission('coordenador') || hasPermission('lider') || hasPermission('direcao') || hasPermission('gestao');
  const shouldShowAdjustButton = () => hasPermission('coordenador') && !plano.isLegacyExecution && plano.status !== 'concluido';
  const shouldShowEditDescricaoButton = () => (hasPermission('admin') || hasPermission('lider') || hasPermission('direcao')) && (plano.status === 'concluido' || plano.status === 'nao_iniciado');
  ...
  const observacao = useMemo(() => {
    if (plano.observacao) return plano.observacao;
    return null;
  }, [plano]);
  ...
  return (
    <>
      ...
      >
        ...
        {plano.os && (
          <p className="text-blue-600 font-semibold text-xs mb-1.5">OS: {plano.os}</p>
        )}
        ...
        <div className="flex gap-2 mt-2 items-center justify-between">
          <div className="flex gap-2 items-center">
            ...
            >
              {activeExecution?.planejamento_id === plano.id ? <Clock className="w-3.5 h-3.5 text-white" /> : (realStatus === 'atrasado' || realStatus === 'replanejado_atrasado') ? <span className="text-white text-xs font-bold">✕</span> : <Play className="w-3.5 h-3.5 text-white" fill="white" />}
            </button>
          ...
          <div className="flex items-center gap-2">
            {shouldShowAdjustButton() ? (
              <button
                onClick={() => { setAdjustedTime(tempoExecutado.toString()); setShowTimeAdjustModal(true); }}
        ...
        <DialogContent>
          ...
          <div className="py-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea id="editDescricao" value={editDescricao} onChange={(e) => setEditDescricao(e.target.value)} placeholder="Digite a descrição da atividade" className="min-h-24" />
            </div>
          </div>
          <DialogFooter>
...
// In your file, keep the previously provided implementations for DailyActivityGroup, ActivityContainer,
// DayCell, MonthView, WeekView, DayView and the main component CalendarioPlanejamento exactly as before,
// ensuring imports at top remain as included in this full file.
//
// ...existing code...